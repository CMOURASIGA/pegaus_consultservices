...
type MessageRow = { id: string; conversation_id: string; role: string; content: string | null; created_at: string; metadata: { correlation_id?: string; attachments?: ChatAttachment[] } | null }

const toConversation = (row: ConversationRow): ChatConversation => ({ id: row.id, title: row.title, updatedAt: row.updated_at })
const toMessage = (row: MessageRow): ChatMessage => ({ id: row.id, conversationId: row.conversation_id, role: row.role === 'assistant' ? 'assistant' : 'user', content: row.content ?? '', createdAt: row.created_at, correlationId: row.metadata?.correlation_id, attachments: row.metadata?.attachments })
...
export class SupabaseChatStore implements ChatStore {
  ...
  async createMessage(input: { ownerId: string; conversationId: string; role: 'user' | 'assistant'; content: string; correlationId: string; provider?: string; model?: string; attachments?: ChatAttachment[] }) {
    ...
    if (!conversation) throw new AppError('CONVERSATION_NOT_FOUND', 'Conversa não encontrada.', 404)
    const { data, error } = await this.client.from('messages').insert({ owner_id: input.ownerId, conversation_id: input.conversationId, role: input.role, content: input.content, content_classification: 'internal', model_provider: input.provider, model_name: input.model, metadata: { correlation_id: input.correlationId, attachments: input.attachments, model_output_trust: input.role === 'assistant' ? 'untrusted' : undefined } }).select('id, conversation_id, role, content, created_at, metadata').single()
