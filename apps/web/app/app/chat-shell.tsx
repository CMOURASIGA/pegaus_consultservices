...
export function ChatShell({ displayName, conversations: initialConversations, activeConversation: initialConversation, initialMessages, initialDraft = '', initialVoiceIntent = false, voiceSurface = false }: Props) {
  ...
  if (voiceSurface) {
    ...
    return <main className="voice-surface" aria-live="polite"><header><a href="/app" onClick={() => cancelVoice(false)}>← Voltar</a><strong>Pegasus</strong><button type="button" onClick={() => cancelVoice()}>Encerrar</button></header><section className="voice-stage"><span className={`voice-hud ${voiceState}`} aria-hidden="true"><i className="voice-hud-ring outer" /><i className="voice-hud-ring middle" /><i className="voice-hud-ring inner" /><i className="voice-hud-core" /></span><p className="eyebrow">CONVERSA POR VOZ</p><h1>{stateLabel}</h1><p>{voiceMessage || 'Use os controles abaixo quando precisar interromper ou iniciar a conversa.'}</p><div className="voice-surface-controls"><button className="voice-main-control" type="button" onClick={toggleVoice} aria-label={voiceState === 'listening' ? 'Concluir gravação' : 'Iniciar conversa por voz'}>{voiceState === 'listening' ? '■' : '●'}</button>{voiceState === 'speaking' ? <button className="secondary-button" type="button" onClick={() => cancelVoice()}>Interromper</button> : null}</div></section></main>
  }
