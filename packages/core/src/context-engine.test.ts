...
class Repository implements MemoryRepository {
  ...
  resolveSourceIdentity?: MemoryRepository['resolveSourceIdentity']
...
describe('ContextEngine', () => {
  ...
  it('marks assistant history as generated context without factual authority', async () => {
    ...
    expect(context.items[0]?.provenance).toMatchObject({ sourceActorType: 'authenticated_user', sourceActorId: 'owner', sourceActorRelationshipToOwner: 'same_as_owner' })
    expect(context.items[1]?.provenance).toMatchObject({ authority: 'assistant_generated', confidence: 0, sourceRef: 'message:assistant-echo', sourceActorType: 'assistant_generated', sourceActorRelationshipToOwner: 'not_applicable' })
  })

  it('provides superseded values only as version history while keeping the current value authoritative', async () => {
    const repository = new Repository([base({ id: 'project', title: 'project:fictional-project', content: 'O projeto fictício agora se chama ProjetoHorizonte.', type: 'project', lastUsedAt: '2026-01-03T00:00:00Z', source: { kind: 'user_message', ref: 'message:user-b' } })])
    ...
    repository.resolveSourceIdentity = vi.fn(async () => ({ type: 'authenticated_user' as const, actorId: 'owner', relationshipToOwner: 'same_as_owner' as const }))
  ...
  it('retrieves authoritative user provenance for a source-author follow-up in a new conversation', async () => {
    const repository = new Repository([base({ id: 'project', title: 'project:fictional-project', content: 'O projeto fictício agora se chama ProjetoHorizonte.', type: 'project', lastUsedAt: '2026-01-03T00:00:00Z', source: { kind: 'user_message', ref: 'message:user-b' } })])
    repository.resolveSourceIdentity = vi.fn(async () => ({ type: 'authenticated_user' as const, actorId: 'user-a', displayName: 'Christian', relationshipToOwner: 'same_as_owner' as const }))

    const context = await new ContextEngine(repository).assemble(request('Quem informou essa mudança?'))

    expect(context.items).toHaveLength(1)
    expect(context.items[0]?.provenance).toMatchObject({
      sourceKind: 'user_message',
      sourceRef: 'message:user-b',
      sourceActorType: 'authenticated_user',
      sourceActorId: 'user-a',
      sourceActorDisplayName: 'Christian',
